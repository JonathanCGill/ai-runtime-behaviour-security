...
| **EC-2.7a** Dry-run / simulation mode | High-risk or first-time actions execute against a sandbox or staging environment before production commit | The gateway routes the action to a simulation target, captures the result, and presents it for human review or Judge evaluation. Only after validation does the action execute against production. Mandatory for irreversible actions in new deployments where no behavioural baseline exists. |
